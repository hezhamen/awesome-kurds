...
export function changeTheme() {
  ...
  const darkAlt = style.getPropertyValue("--dark-alt");

  const light = style.getPropertyValue("--light");
  const lightAlt = style.getPropertyValue("--light-alt");

  document.documentElement.style.setProperty("--dark", light);
  document.documentElement.style.setProperty("--dark-alt", lightAlt);

  document.documentElement.style.setProperty("--light", dark);
  document.documentElement.style.setProperty("--light-alt", darkAlt);
}
...
export function getPhoto(kurd: Kurd) {
  ...
  if (/twitter.com/gi.test(kurd.link)) {
    ...
    image =
      `https://res.cloudinary.com/mhmd-azeez/image/twitter_name/${username}.jpg`;
